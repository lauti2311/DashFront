...
const ModalProducto: React.FC<ModalProductProps> = ({
    ...
    articuloManufacturadoDetalles: productToEdit?.articuloManufacturadoDetalles ? productToEdit.articuloManufacturadoDetalles.map((detalle: any) => {
      ...
      return {
        ...
        articuloInsumo: {
          ...
          unidadMedida: articuloInsumo.unidadMedida || "", // Asegúrate de mapear todas las propiedades correctamente
          precioCompra: articuloInsumo.precioCompra || 0,
          stockActual: articuloInsumo.stockActual || 0,
          stockMaximo: articuloInsumo.stockMaximo || 0,
          stockMinimo: articuloInsumo.stockMinimo || 0,
          esParaElaborar: articuloInsumo.esParaElaborar || false,
          categoria: {
            id: categoria.id || 0,
          ...
          imagenes: articuloInsumo.imagenes || [], // Asegúrate de incluir la propiedad 'imagenes'
        },
      };
    }) : [],
  ...
  const fetchUnidadesMedida = async () => {
    try {
      const unidades = await unidadService.getAll(`${url}unidadesmedida`);
