...
export default class ArticuloInsumoShortService extends BackendClient<ArticuloInsumoShortDto> {
    public async insumosParaElaborar(url: string, idSucursal: number): Promise<ArticuloInsumoShortDto[]> {
        try {
    ...
    public async insumos(url: string, idSucursal: number): Promise<ArticuloInsumoShortDto[]> {
        try {
